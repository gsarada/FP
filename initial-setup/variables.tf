...
# Legacy - AWS relies on its own library of trusted root certificate authorities (CAs) for validation instead of using any configured thumbprints
...
variable "github_url" {
  description = "GitHub OpenID provider URL."
  type        = string
...
variable "repositories" {
  ...
  validation {
    ...
    condition = length([
      ...
      if length(regexall("^[A-Za-z0-9_.-@]+/[A-Za-z0-9_.-@]+:([A-Za-z0-9_.:/-]+|\\*)$", repo)) > 0
...
variable "ghtags" {
  ...
  type        = map(string)
  default     = {"Name":"GitHub Actions Deploy Role", "ManagedBy":"terraform"}
}
