...
app = FastAPI(title="Financial Planner Researcher Service")
...
async def health():
    ...
    return {
        "service": "FP Researcher",
        "status": "healthy",
        "ingest_api_configured": bool(os.getenv("INGEST_API_ENDPOINT") and os.getenv("INGEST_API_KEY")),
        "timestamp": datetime.now(UTC).isoformat(),
        "debug_container": container_indicators,
        "aws_region": os.environ.get("AWS_DEFAULT_REGION", "not set"),
        "bedrock_model": os.environ.get("RESEARCHER_MODEL", "bedrock/global.openai.gpt-oss-120b-1:0"),
